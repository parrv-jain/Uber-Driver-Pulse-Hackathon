  ...
  return (
    <aside style={{
      ...
      <div style={{ padding:'0 20px 24px', borderBottom:'1px solid var(--border)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily:'var(--font-mono)', fontSize:9, color:'var(--text-3)', letterSpacing:'2px', textTransform:'uppercase' }}>
              {isAdmin ? 'Admin Panel' : 'Pulse v2'}
            </div>
          </div>
        </div>
      </div>
      ...
      <nav style={{ flex:1, padding:'16px 10px' }}>
        <div style={{ fontFamily:'var(--font-mono)', fontSize:9, letterSpacing:'3px', textTransform:'uppercase', color:'var(--text-3)', padding:'0 10px', marginBottom:8 }}>
          {isAdmin ? 'Uber Admin' : 'Navigation'}
        </div>
        {items.map(item => {
          const isActive = active === item.id;
          const activeColor = isAdmin ? 'var(--cyan)' : 'var(--lime)';
          return (
            <div key={item.id}
      ...
      <div style={{ padding:'16px 20px 0', borderTop:'1px solid var(--border)' }}>
        {isAdmin ? (
          <div style={{ background:'rgba(53,212,241,0.08)', border:'1px solid rgba(53,212,241,0.2)', borderRadius:'var(--r)', padding:'12px 14px' }}>
            <div style={{ fontSize:13, fontWeight:700, marginBottom:3, color:'var(--cyan)' }}>Uber Admin</div>
            <div style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--text-3)', display:'flex', alignItems:'center', gap:5 }}>
              <span style={{ width:6, height:6, borderRadius:'50%', background:'var(--cyan)', display:'inline-block' }}></span>
              Operations Access
            </div>
          </div>
        ) : driver ? (
